...
| 10 | P15-024 | [P15-024-context-artifact-exclusion.md](../tasks/P15-024-context-artifact-exclusion.md) | **pending** | Context-builder artifact exclusion (B021) — extend `SKIP_DIRS` with `.next`/`out`/`out_deploy`/`.specstory`/`.turbo`/`coverage`/`.svelte-kit`/`.nuxt` + env override `MCP_CODER_CONTEXT_EXCLUDE_DIRS`. Fixes P15-ISS-024 (high). Prerequisite for P15-ISS-028 (delete no-spec path) + P15-ISS-026 (hard preflight gate). Epic 5 dogfood. |
| 11 | (Phase C) | _(spec TBD)_ | **pending** | Collapse to one executor path — delete the no-spec `else:` branch (P15-ISS-028) + hard preflight gate (P15-ISS-026). Depends on P15-024. Epic 5 dogfood. |
