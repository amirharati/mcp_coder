...
| 11 | P15-025 | [P15-025-delete-semantics.md](../tasks/P15-025-delete-semantics.md) | **pending** | Deterministic file deletion (B022/B023) — add `files_delete` contract (`### Delete` subsection / YAML) + post-executor engine-managed removal (`git rm` / `os.remove`) captured in `delta.deleted`. Spec-backed path only. Fixes P15-ISS-025 (high). User critical path for resuming `idealabs_web` Epic 5 delete work. |
| 12 | (Phase C) | _(spec TBD)_ | **pending** | Collapse to one executor path — delete the no-spec `else:` branch (P15-ISS-028) + hard preflight gate (P15-ISS-026). Depends on P15-024. Epic 5 dogfood. |
