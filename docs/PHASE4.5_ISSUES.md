...
| P4.5-ISS-015 | open | low | **Multi-language symbol scan + outlines (polyglot repos)** | Symbol discovery limited to 9 extensions; repo-map/excerpt regex is Python-only. C/C++/Go/Rust/etc. work via spec paths but not auto-discovery. **Later (BL-352):** widen `SCAN_EXTENSIONS`, optional config, per-language outline heuristics; path to BL-348. Goal: good enough for common monorepo cases without listing every file in spec. **Phase 5+**. |
...
| Date | Change |
|------|--------|
| 2026-06-10 | P4.5-ISS-015 filed: multi-language symbol scan + outlines (C/C++/Go/Rust…) → BL-352. T-04 SCAN_EXTENSIONS Q&A. |
