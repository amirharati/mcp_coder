...
| P8-ISS-004 | done | low | Thinking token availability: need to verify litellm passes thinking blocks through to `ModelResponse` for all providers we use (Anthropic extended thinking, OpenRouter reasoning) | **Verified in live dogfood:** complex Sonnet run (`86fe232f-3bdd-4d04-a9e3-bdcbd3d8ce63`) still produced no `thinking_text`/`thinking_tokens` fields on `backend_llm_call`. Treat as provider/path-dependent behavior (not a Phase 8 blocker). Keep schema and capture path as-is; revisit when running a backend/model path that explicitly emits reasoning blocks/tokens. |
...
| Date | Change |
|------|--------|
| 2026-06-14 | P8-ISS-004 closed as non-blocking after A5 complex dogfood: capture path works, but Sonnet/OpenRouter run does not expose reasoning fields. |
