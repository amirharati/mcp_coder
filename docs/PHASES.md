...
| **11** *(milestones done)* | **Supervised execution + smarter context** — `SupervisedIO` + `DelegationSupervisor` (replace `yes=True`); pre-delegation clarity pass; executor-pull `/read` hint; mid-run human gate (experimental); tier-1 post-executor reviewer; smart planner trigger; host `model_policy` arg (BL-512 Stage 2); `architect_pass` → `planner_pass` naming refactor | [PHASE11_MVP.md](./PHASE11_MVP.md) — P11-001..P11-008 all **done** 2026-06-19; pre-dogfood gate open |
