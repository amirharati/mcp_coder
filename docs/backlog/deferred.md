<!--
  STEWARDSHIP — deferred / partial / idea backlog items.

  LLM: grep "^### BL-NNN" deferred.md — do not read whole file.
  EDIT: planning session only. On change → sync ../BACKLOG.md index row + changelog.
  SHIP: cut section → done.md; index status → done. Recipes: README.md § For LLMs.
  Workers: do not edit; propose in task § Results.
-->

# Deferred backlog — full text

Full text of open, partial, and idea backlog items. **Read [../BACKLOG.md](../BACKLOG.md) first** for the index (one-liners + status). Shipped items: [done.md](./done.md). **How to update:** [README.md](./README.md) § For LLMs.
...
### BL-535: Trace inspect UX — specless CLI runs via trace-file fallback when DB index missing

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Trace inspect UX


### BL-536: Role attribution completeness — every proxy/backend LLM call has role/model/provider/ok/duration

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Role attribution completeness


### BL-537: Reviewer semantics — explicit `reviewer_mode`/`reviewer_outcome`/`reviewer_action` in outputs

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Reviewer semantics


### BL-538: Planner audit normalization — same shape as clarity/spec-validation/reviewer blocks

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Planner audit normalization


### BL-539: Clarity telemetry polish — `clarity_round_index`/cap/auto_passed in trace + records

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Clarity telemetry polish
...
### BL-553: Escalation pause resume semantics — verify mid-loop `needs_input` / `max_turns_reached`

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** After P13-016, clarity-block pauses auto-resume on host return (no `answer` needed). Escalation pauses (supervisor mid-loop `needs_input` / `max_turns_reached`) remain answer-gated, but we have no live examples yet to confirm correct behavior. Need: (1) dogfood scenarios that trigger escalation pauses, (2) verify resume path (with and without `answer`), (3) define proper behavior once we have a few real examples. May also require updates to the agent itself. Deferred (not Phase 13).
...
**Notes:** Beyond single 3-step budget: task- and model-sensitive compile (tiers, excerpting, transcript, map depth); integrate with context builder; bust/reuse executor cache correctly on model change; auto-select policy per delegation with minimal user config (BL-162). From P4.5-ISS-010 — Phase 5+.


### BL-348: Beyond file-summary RAG

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).


### BL-349: Recently touched files — session + project, git + manifest fusion

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Today: per-delegation `files_changed` (manifest walk ± git dirty), `get_file_history` (one file → timeline), `list_delegations` — no aggregated “recently updated files” view, no MCP `recent_files` / picker hint, no merge of git mtime/status with `workspace_history` file deltas. Later: rank paths touched in current MCP session vs project-wide N delegations / time window; attach detail (checkpoint summary, diff snippet, BL-348 symbol summary) when relevant to task/RAG/symbol query; surface to planner MCP + context picker/builder as read hints (not auto edit). Phase 5+. From P4.5-ISS-012.
...
**Notes:** P5-ISS-004 — `spec_validation` block → no `rag_retrieval` / empty `context_refs` (by design; confusing in logs). Log skip reason on blocked delegates. Phase 5+ / BL-353-5a. Repro: session `1432fc02…` #2–#4.
...
### BL-301: Delegation log web UI

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Extend viewer for `~/.mcp-coder`


### BL-302: Redaction policy doc for logs (secrets)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Required before sharing logs


### BL-303: Metrics export (Prometheus / statsd)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Enterprise-ish; low priority


### BL-304: Global index `hosts/cursor/<id>/index.json`

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Cross-project session lookup — P1-ISS-008 (`carried` at P1-199); one Cursor chat delegating to multiple repos


### BL-306: Startup code version / git hash in MCP stderr

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Detect stale process (P1-ISS-009)


### BL-307: `MCP_CODER_SINGLETON=all` aggressive global kill

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Escape hatch; default stays per-workspace


### BL-308: Global `server.jsonl` locking / per-pid subfiles

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** P1-ISS-011; only if garbled lines in practice
...
### BL-549: Viewer full-fidelity output mode (no digest-only blind spots)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Add an explicit full-fidelity mode in `mcp-coder view delegations` that resolves and shows complete available payloads (row + trace bodies) with provenance labels, so operators can inspect raw executor/reviewer text without losing context to digest-only previews. Deferred (not Phase 13).


### BL-550: Output truncation policy controls + clear UX labeling

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Make truncation policy configurable (`no-truncate`/higher caps) and clearly label whether text is server-truncated (`…[truncated]`) vs UI preview-clipped. Goal: avoid confusion between data loss and display clipping in viewer/log workflows. Deferred (not Phase 13).


### BL-551: Pointer-first short log + viewer resolve-to-full flow

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Rework `delegations.jsonl` short rows to be pointer-first (`trace_ref`/hash/bytes) and avoid duplicated inline text where possible; viewer should resolve full text from pointers on demand with fallback when detail blobs are missing. Deferred (not Phase 13).


### BL-552: Logging policy review — default full-fidelity capture + retention tradeoffs

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Review whether default behavior should capture 100% of available delegation/trace content for debugging, with storage/GC policy handling disk growth separately (rather than relying on lower-fidelity defaults). Deferred (not Phase 13).


### BL-554: Executor error classification — confirm non-config for edit-flow failures (watch)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** When a delegation fails with executor error (not clarity block, not success), inspect `error_detail.error_class` in the delegation row. Confirm it is NOT `config` when the failure was edit/patch/format related (even if `NotFoundError` appears). Confirm true config/auth errors still classify as `config`.


### BL-555: Unknown loop failure typed cause — confirm row/view fields populated (watch)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** When a delegation ends with `supervisor_turn_end.worker_outcome=unknown` or `supervisor_loop_end.end_reason=unknown`, confirm the delegation row carries `error_detail.error_class=unknown` + `error_message` (e.g. `supervisor_loop_unknown`), and the viewer enrichment surfaces `outcome`/`error_class`/`error_message` in the `mcp→host` event.


## Executor & backends

### BL-004: OpenCode adapter (subprocess)
...
### BL-160: Full interactive via CLI

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** `mcp-coder session` — same core as MCP, no Cursor transport; multi-turn chat with executor in terminal.
...
### BL-162: Router / janitor + cheap model for context build
...
### BL-201: Claude Desktop host adapter

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Low priority — after Cursor + owned context


### BL-202: Windsurf / other IDEs
...
### BL-204: Proxy intercept: save latest Cursor prompt from `context_optimizer_proxy`

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Personal workflow


### BL-205: Cursor rule / skill snippet for routing to `delegate_to_agent`

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Improve auto-routing
...
### BL-344: Configurable spec granularity (step size / full-epic delegate)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Workspace config + user opt-in: fine-grained step specs vs larger “big step” specs vs single task covering whole epic (`spec_path` = epic-scale contract). Unlock higher-end models that can do more per delegate; requires experiments on context, scope gateway, reports, versioning. From P4.5-ISS-007 — Phase 5+, not 4.5.


### BL-345: Mechanical spec lint (pre-delegate, no LLM)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** `mcp-coder spec lint` + optional delegate gate: required sections, non-empty Goal/Files, placeholder detection, empty `prompt_block` → `invalid_spec`; optional on-disk path warnings. Tiers: warn vs `spec_lint: strict`. Complements `spec_validation` (chat LLM) and `mode=review` (brainstorm). From P4.5-ISS-008 — Phase 5+.


### BL-346: Model-aware context budget defaults + cap enforcement

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Per-executor-model input budget defaults (window minus output/overhead reserve); clamp `MCP_CODER_CONTEXT_BUDGET_TOKENS` / yaml overrides to model max; maintain `context_budget_tokens` via provider/OpenRouter refresh or periodic update (today hand-edited `model_rates.yaml`). From P4.5-ISS-009 — Phase 5+.
...
### BL-361: "One step at a time" / always-review-before-implement delegate mode
...
### BL-001: Owned context pipeline (summarize, rank, trim)
...
### BL-003: Router / janitor LLM inside mcp-coder
...
### BL-005: Dual-mode CLI (`mcp-coder run …`)
...
### BL-006: Context janitor, critic, test-writer sub-agents
...
### BL-008: Skills injection library
...
### BL-009: Explicit MCP tools: `continue_session`, `get_session_status`
...
### BL-010: ~~DB-backed session persistence~~
...
### BL-151: Gatekeeper MCP for protected specs

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** [OTEHR_RELATED_IDEAS/GATEKEEPING_MCP.md](./OTEHR_RELATED_IDEAS/GATEKEEPING_MCP.md) — still deferred post-P1-151


### BL-152: Mirror `delegations.jsonl` + reports in product UX

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Partial: `specs/reports/` mirrors delegation audit; viewer TBD


### BL-161: Multi-agent inside MCP (planner → executor)

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Single MCP tool call from Cursor still triggers an internal pipeline: architect/planner pass (steps, file plan, risks) then executor (Aider). Cursor stays thin; mcp-coder owns substeps, logs each phase.


### BL-360: Code layout refactor — instance sub-folders, file size audit

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** `core/engine/` and `core/host/` mix abstract factories with concrete implementations in flat folders. Later: `core/engine/backends/aider/`, `core/host/hosts/cursor/`; audit + split files > ~400 lines (`server/mcp_server.py` ~1750 today). Pure structural; no behavior change. Phase 5+. From P4.5-ISS-001.


### BL-401: `always_new` vs `align_host`

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Default `MCP_CODER_SESSION_POLICY` — partial: E2E favors `align_host` for test sandbox; keep default `always_new` globally


### BL-403: Prompt size vs failure rate per model

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Deferred at P1-199; transcript cap partial P1-140


### BL-404: Cursor `target_files` reliability

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** Schema / inference rules


### BL-405: Tool name/description for routing

**Status:** `deferred` — index/table entry (no standalone section in pre-split backlog).

**Notes:** MCP tool description


### BL-501: If human latency exceeds MCP timeout, persist “awaiting_host” state + resume token (BL-501 adjacency)
