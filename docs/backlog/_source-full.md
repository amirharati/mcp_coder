...
| BL-549 | **Viewer full-fidelity output mode (no digest-only blind spots)** | Add an explicit full-fidelity mode in `mcp-coder view delegations` that resolves and shows complete available payloads (row + trace bodies) with provenance labels, so operators can inspect raw executor/reviewer text without losing context to digest-only previews. **Deferred (not Phase 13).** |
| BL-550 | **Output truncation policy controls + clear UX labeling** | Make truncation policy configurable (`no-truncate`/higher caps) and clearly label whether text is server-truncated (`…[truncated]`) vs UI preview-clipped. Goal: avoid confusion between data loss and display clipping in viewer/log workflows. **Deferred (not Phase 13).** |
| BL-551 | **Pointer-first short log + viewer resolve-to-full flow** | Rework `delegations.jsonl` short rows to be pointer-first (`trace_ref`/hash/bytes) and avoid duplicated inline text where possible; viewer should resolve full text from pointers on demand with fallback when detail blobs are missing. **Deferred (not Phase 13).** |
| BL-552 | **Logging policy review: default full-fidelity capture + retention tradeoffs** | Review whether default behavior should capture 100% of available delegation/trace content for debugging, with storage/GC policy handling disk growth separately (rather than relying on lower-fidelity defaults). **Deferred (not Phase 13).** |
| BL-553 | **Escalation pause resume semantics — verify + define proper behavior** | After P13-016, clarity-block pauses auto-resume on host return (no `answer` needed). Escalation pauses (supervisor mid-loop `needs_input` / `max_turns_reached`) remain answer-gated, but we have no live examples yet to confirm correct behavior. Need: (1) dogfood scenarios that trigger escalation pauses, (2) verify resume path (with and without `answer`), (3) define proper behavior once we have a few real examples. May also require updates to the agent itself. **Deferred (not Phase 13).** |

---

## Watch for evidence (fixed-pending-verify → confirm in future runs)

Items below have code fixes + regression tests in place but lack live dogfood evidence to close fully. **When running log analysis or dogfood in future sessions, check these conditions and close the source issue if confirmed.**

| BL | Watch item | What to check | Source issue |
|----|-----------|---------------|--------------|
| BL-554 | **Executor error classification — confirm non-config for edit-flow failures** | When a delegation fails with executor error (not clarity block, not success), inspect `error_detail.error_class` in the delegation row. Confirm it is NOT `config` when the failure was edit/patch/format related (even if `NotFoundError` appears). Confirm true config/auth errors still classify as `config`. | [P13-ISS-006](./PHASE13_ISSUES.md) |
| BL-555 | **Unknown loop failure typed cause — confirm row/view fields populated** | When a delegation ends with `supervisor_turn_end.worker_outcome=unknown` or `supervisor_loop_end.end_reason=unknown`, confirm the delegation row carries `error_detail.error_class=unknown` + `error_message` (e.g. `supervisor_loop_unknown`), and the viewer enrichment surfaces `outcome`/`error_class`/`error_message` in the `mcp→host` event. | [P13-ISS-015](./PHASE13_ISSUES.md) |
